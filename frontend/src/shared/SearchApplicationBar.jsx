...
const SearchApplicationBar = () => {
    ...
    const searchHandler = async()=>{
        ...
        const res = await fetch(`${BASE_URL}/application/search/getApplicationBySearch?receipt=${receipt}`)

        if(res.success) alert ('Something went wrong');

        const result = await res.json();

        navigate(`/application/search?receipt=${receipt}`, {state: result.data})
   };
...
export default SearchApplicationBar